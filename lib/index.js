	...
	return {
		visitor: {
			JSXOpeningElement: function JSXOpeningElement(path, state) {
				...
				if (componentNameAttrIndex == null && htmlTagsArray.indexOf(name.name) !== -1) {
					return;
				}

				attributes.push(makeAttribute(generateName(path.container.openingElement.name.name, implicitRole), state.opts.attributeName));
			}
		}
	};
};